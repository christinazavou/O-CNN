...
DEFINE_float(last_label,kOptional,-1.0, "Value to set undetermined label, set to -1(do nothing)");
...
class OctreeBuilder {
 public:
  ...
  void build_octree(float last_label) {
    octree_.build(octree_info_, point_cloud_,last_label);
  }
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < all_files.size(); i++) {
    ...
    for (int v = 0; v < FLAGS_rot_num; ++v) {
      ...
      // build
      builder.build_octree(FLAGS_last_label);
